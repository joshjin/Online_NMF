...
function [W,H,iter,HIS]=nmf(A,k,varargin)
    ...
    if par.verbose          % collect information for analysis/debugging
        % calculate the initial gradient of W, H
...
%------------------------------------------------------------------------------------------------------------------------------
    ...
    % end of setting up for history collection
...
%                 0 - None (want to stop by MAX_ITER or MAX_TIME)
    ...
    [m,k]=size(W);, [k,n]=size(H);, numAll=(m*k)+(k*n); %#ok<ASGLU>
    switch stopRule
        case 1
